...
mid_freq  = 10
omegas    = concatenate((logspace(log10(min_freq),log10(mid_freq+10),150),linspace(mid_freq,max_freq_to_analyze,140)))
...
modelL.add(Debye([.01 , 20 ] ,[(.000001,.3) , (.1 ,150)] , "2nd Debye"))
#modelL.add(Debye([.01 , 150] ,[(.000001,.3) , (.1 ,300)], "3rd Debye"))
...
#modelL.add(DHO([.3,760,200]  ,[(.0001,2) ,(650,800)  ,(1,1000)]," L3"))
...
modelL.add(BrendelDHO([.1,750,244,10]  ,[(.001,.4) ,(450,800)  ,(1,1000),(1,100)],"Brendel L1"))
modelL.add(BrendelDHO([.3,760,200,10]  ,[(.01,2) ,(650,800)  ,(1,1000),(1,100)],"Brendel L2"))
modelL.add(BrendelDHO([.1,450,100,10]  ,[(.001,.4) ,(380 ,700)  ,(.1,400),(1,100) ],"Brendel L3"))
...
print("longitudinal:")
modelL.print_model_latex()
print("transverse:")
modelT.print_model_latex()    


#-------------------- priting out parameters and plotting ------------------------------
print_gLST_LHS_stuff(modelL, modelT, rp, cp)

plot_model(modelL,omegas,Ldatarp,Ldatacp,1,.001,max_freq_to_analyze,title='Longitudinal',longitudinal=True) 
plot_model(modelT,omegas,rp,cp,2,.001,max_freq_to_analyze,title='Transverse')

plot_model(modelL,omegas,Ldatarp,Ldatacp,3,.001,max_freq_to_analyze,xscale='log',title='Longitudinal',longitudinal=True) 
plot_model(modelT,omegas,rp,cp,4,.001,max_freq_to_analyze,xscale='log',title='Transverse') 

end_t = time.time()
m, s = divmod(end_t - start_t, 60)
h, m = divmod(m, 60)
print("%02d hr %02d min %02d" % (h, m, s))
